...
# and probably some good looking girl or boy you want to marry.
...
for shop in (freelancers, antiques, pet_shop):
    # input box to show what you can buy...capture text-string of what was bought...
    buy_item = input(
        f'Welcome to {shop["name"]}! what do you want to buy: {shop}. If you wish to skip store type "exit" \n').lower()
    # update the cart
    if (buy_item == "exit"):
        continue
        ...
        continue
    # update string
    buy_items_string = buy_items_string + f'{buy_item} : {shop[buy_item]} Gp, '
    # update cart
    cart.update({buy_item: shop.pop(buy_item)})
    items_purchased = ", ".join(list(cart.keys()))
    total_sum = sum(cart.values())
    print(f'Store name {shop["name"]}, inventory after purchase: {shop}')


print(f'You Purchased {buy_items_string}. Your total is {total_sum} Gp. Your change is {purse - total_sum} Gp. Have a nice day of mayhem!')
